...
def getReposList()
{
    ...
    repos.add(getRepoMap('foo','foo','test1155'))
    repos.add(getRepoMap('bar','bar','test1155'))
    repos.add(getRepoMap('baz','baz','test1155'))
    repos.add(getRepoMap('subfoo','subfoo','test1155'))
    repos.add(getRepoMap('subbar','subbar','test1155'))
    return repos
}
...
def finishRelease(String branch, String extraBranch)
{
    ...
    if (success > 0) {
        ...
        if (extraBranch == null) {
            telegramMessage = "Branch `${branch}` merged into `master` and `develop` [[${success}/${total}]]"
        } else {
            telegramMessage = "Branch `${branch}` merged into `master`, `develop` and `${extraBranch}` [[${success}/${total}]]"
        }
        telegramSend(
